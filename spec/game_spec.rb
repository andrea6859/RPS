...
describe Game do
  ...
  let(:options) { {"name" => "Andrea", "player_weapon" => "scissors",  "computer_weapon" => "paper"} }
  ...
  context 'end game' do
    ...
    let(:draw_options) { {"name" => "Andrea", "player_weapon" => "paper",  "computer_weapon" => "paper"} }
      ...
      describe '#lose?' do
        it 'returns false if player_weapon is :rock and computer_weapon is :paper' do
        expect(lose_game.lose?).to eq true
      end
      end

      describe '#win?' do
        it 'returns true if player_weapon is :scissors and computer_weapon is :paper' do
        expect(win_game.win?).to eq true
      end
      end

      describe '#draw?' do
        it 'returns true if player_weapon is :paper and computer_weapon is :paper' do
